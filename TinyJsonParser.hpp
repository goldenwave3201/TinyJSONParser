#ifndef TINY_JSON_PARSER_HPP_
#define TINY_JSON_PARSER_HPP_
...
namespace TinyJsonParser
{
	class Value
    {
	    public:
            ...
            R GetAs()
            {
                std::istringstream iss(value_); //read string
                R v;
                iss >> v;
                return v;
            }
            ...
            void Set(V v)
            {
                std::ostringstream oss; //write string
                if (nokey_) {
                        oss << v;
                }
                else {
        ...
        private:
            std::string value_;  //define string type
            bool nokey_;         //judge json key is null
	};
